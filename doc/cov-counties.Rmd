...
# fast random forests
library(ranger)
# missing data imputation using ranger
# library(missRanger)
...
What is common in the machine learning is to keep our predictive models "honest" by estimating its performance on data the model was not trained with.
...
We'd like to understand how county characterics explain some of the variation in covid cases.  We include the predicted values to examine what additional county covariates are significant in explaining the remaining variation.

```{r results="hide"}
# county_covars <- read_csv("https://ond3.com/covdata/county_covars.csv")
# county_covars <- read_csv("./Data/county_covars.csv")
...
tibble(county_name = names(residuals(goog_mdl)), res = residuals(goog_mdl)) %>%
  ...
  column_to_rownames("county_name") %>%
  lm.spike(res ~ ., data = ., niter = spike_slab_iter * 10) -> ss_mdl
```
...
The additional covariates explain `r round(ss_rsq*100)`% of the remaining variance in the residuals.
...
covar_dat <- nber_dat %>%
  ...
  select(jhu_csse_deaths_incidence_num_value, predicted,
         pct_65_and_older:year_structure_built) %>%
...
```{r}
ss_mdl <- lm.spike(jhu_csse_deaths_incidence_num_value ~ ., niter = 10000,
                   data = covar_dat)
```
...
#covidcast <- read_csv("./Data/cmu_covidcast.csv",
                      col_types = cols(
...
```{r}
fcst <- covid_ts %>% filter(date < "2020-06-01") %>%
